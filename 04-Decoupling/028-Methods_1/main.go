...
// There is a named type from the net package called IP and IPMask with a base
...
// And is using a value recorder and returning a value of type Time.
// This method is using value semantics for Time.

func (t Time) Add(d Duration) Time {
	t.sec += int64(d / 1e9)
	nsec := int32(t.sec) + int32(d%1e9)
	if nsec >= 1e9 {
		t.sec++
		nsec -= 1e9
	} else if nsec < 0 {
